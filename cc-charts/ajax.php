...
use CryptocurrencyCharts\Config;
use CryptocurrencyCharts\Helper;
...
class Ajax {
  ...
  const DATA_URL            = 'http://3aindex.ru/cc-charts/data/GetData.php?fsym=%s&tsym=%s&limit=%d&aggregate=%d&allData=%s&code={sgXdYvgL';
  const COINS_LOGOS_FOLDER  = 'assets/images/coins';

  public function __construct() {
    $this->pluginDir = CCC_ROOT_DIR . '/';
    $this->env = Config::get();
  }

  public function ajaxGetData() {
    $marketData = [];
    $assetId = isset($_REQUEST['asset']) ? $_REQUEST['asset'] : NULL;
    $currency = isset($_REQUEST['currency']) ? $_REQUEST['currency'] : NULL;
    $assetSymbol = isset($this->env->coins->$assetId->symbol) ? $this->env->coins->$assetId->symbol : NULL;

//    if ($assetId && $assetSymbol && $currency) {
    if ($assetId && $assetSymbol) {
      // check if data can be retrieved from cached file or needs to be retrieved from server
      $cacheFilePath = self::DATA_FOLDER . '/' . $assetId . '_' . $currency . '.json';
      // if cache file with data exists and not expired
      if (file_exists($this->pluginDir . $cacheFilePath) && time() - filemtime($this->pluginDir . $cacheFilePath) <= $this->env->cacheTime) {
        Helper::log(sprintf('%s: cache file exists', $assetSymbol));
        $marketData = Helper::loadJSON($cacheFilePath);
        // if cache file not exists or expired
      } else {
        Helper::log(sprintf('%d %s: no cache file present', $assetId, $assetSymbol));
        //retrieve data from API
        $url = sprintf(self::DATA_URL, $assetSymbol, $currency, 2000, 1, 'false');
        // https://min-api.cryptocompare.com/data/histoday?fsym=1182&tsym=USD&limit=2000&aggregate=1&allData=false
        Helper::log($url);
        ...
        if ($http->statusOk()) {
          $json = json_decode(Helper::cleanString($http->getBody()), JSON_OBJECT_AS_ARRAY);
          if (isset($json['Data'])) {
            if ($rowsCount = count($json['Data'])) {
              Helper::log(sprintf('%d rows retrieved from API', $rowsCount));
              $marketData = $json['Data'];
              // convert to JS unix timestamp format
              array_walk($marketData, function (&$item) {
                $item['time'] *= 1000;
                $item['volume'] = $item['volumefrom'];
                $item['value'] = $item['close'];
                unset($item['open']);
                unset($item['high']);
                unset($item['low']);
                unset($item['close']);
                unset($item['volumefrom']);
                unset($item['volumeto']);
              });
              // save data to cache file
              Helper::saveJSON($cacheFilePath, $marketData);
            } else {
              Helper::log('No data retrieved from API');
            }
          }
        } else {
          Helper::log(sprintf('HTTP Error: %s', $http->getBody()));
        }
      }
    }

    Helper::log(sprintf('%d rows loaded', count($marketData)));
    if (count($marketData)) {
      echo json_encode([
        'success' => TRUE,
        'data' => $marketData,
  ...
  public function ajaxSymbolAutocomplete() {
    $result = [];
    $searchTerm = isset($_REQUEST['q']) ? strtolower($_REQUEST['q']) : '';
    if ($searchTerm) {
      foreach ($this->env->coins as $id => $coin) {
        if (strpos(strtolower($coin->symbol), $searchTerm) !== FALSE || strpos(strtolower($coin->name), $searchTerm) !== FALSE) {
          $result[] = [
            'id' => $id,
            'symbol' => $coin->symbol,
            'text' => $coin->name,
            'logo' => self::COINS_LOGOS_FOLDER . '/thumb30/' . $coin->logo
          ];
        }
      }
    }
